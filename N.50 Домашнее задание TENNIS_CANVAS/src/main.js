...
import { createCANVAS } from './createGameCANVAS';

var hash = createCANVAS();
...
function tick() {
	...
	// мяч правее стены?
	if ((ballOptions.posY < racketOption.r2PosY
		|| ballOptions.posY > (racketOption.r2PosY + racketOption.height))
		&& ballOptions.posX + ballOptions.radius >= (positionLeft + positionWidth)) {
		player1 += 1;
		scoreBoardInnerHTML();
		ballOptions.speedX = 0;
		ballOptions.speedY = 0;
		ballOptions.posX = positionLeft + positionWidth - ballOptions.radius - 1;
	} else if (!(ballOptions.posY < racketOption.r2PosY
		|| ballOptions.posY > (racketOption.r2PosY + racketOption.height))
		&& ballOptions.posX + ballOptions.radius > (racketOption.r2PosX)) {
		ballOptions.speedX = - ballOptions.speedX;
		ballOptions.posX = positionLeft + positionWidth - racketOption.width - ballOptions.radius;
	}

	// мяч левее стены?
	if ((ballOptions.posY  < racketOption.r1PosY
		|| ballOptions.posY > (racketOption.r1PosY + racketOption.height))
		&& ballOptions.posX - ballOptions.radius <= (positionLeft)) {
		player2 += 1;
		scoreBoardInnerHTML();
		ballOptions.speedX = 0;
		ballOptions.speedY = 0;
		ballOptions.posX = positionLeft + ballOptions.radius+1;
	} else if (!(ballOptions.posY < racketOption.r1PosY
		|| ballOptions.posY > (racketOption.r1PosY + racketOption.height))
		&& ballOptions.posX - ballOptions.radius < (racketOption.width + racketOption.r1PosX)) {
		ballOptions.speedX = - ballOptions.speedX;
